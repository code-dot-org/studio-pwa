/**
 * localStorage adapter for seat storage.
 *
...
export async function readSeatProgress(
  seatId: SeatId,
): Promise<AiDecisionsProgress | null> {
  return prefsGet<AiDecisionsProgress>(keySeatProgress(seatId));
}

export const PROGRESS_UPDATED_EVENT = 'mobile:progress-updated';

export async function writeSeatProgress(
  progress: AiDecisionsProgress,
): Promise<void> {
...
export async function readNotebookProgress(
  seatId: SeatId,
): Promise<NotebookProgress | null> {
  const seat = await readSeatProfile(seatId);
  if (seat === null) return null;
  const p = seat.journeys?.['notebook'];
  if (!p || p.kind !== 'notebook') return null;
  return p;
}

export async function writeNotebookProgress(
  seatId: SeatId,
