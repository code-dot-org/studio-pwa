...
function BinsMatchRenderer({
  ...
  const promptText = payload.promptKey ? getString(payload.promptKey) : '';
  const doneText = getString(level_done_key(payload));
  ...
  return (
    <Box sx={{display: 'flex', flexDirection: 'column', gap: 2, padding: 2}}>
      <Box sx={{display: 'flex', alignItems: 'flex-start', gap: 1}}>
        ...
        >
          {allDone ? doneText : promptText}
        </Typography>
        {!allDone && promptText && (
          <SpeakerAffordance text={promptText} ariaLabel="Read prompt aloud" />
        )}
      </Box>
